...
//    }
    ...
    double denom1 = (-sqrt_f1d1 * (exp(-sqrt_f1d1 * non_growing_final) -
                                   exp(sqrt_f1d1 * non_growing_final)));
    double denom2 = sqrt_f2d2 * (-exp(-sqrt_f2d2 * sqrt_f2d2) +
                                  exp(sqrt_f2d2 * non_growing_final - 2 * sqrt_f2d2 * (length_growing_initial)));

    double A1 = 0.0; // coefficient for theta1 = A1 *(exp(-...x) +exp(...x))
    double A2 = 0.0; // coefficient for theta2 = A2 *(exp(-...x) +exp(...x))
    ...
    for (int i = 1; i < n; i++) {
        ...
        for (int j = 0; j < solution_grid - int(non_growing_final * double(solution_grid)); j++) {
            an(i, j + int(non_growing_final * double(solution_grid))) =
                    2 * C0 / (i * M_PI) * sin(i * M_PI * beta_grow / length_growing_initial);
        }
    ...
    for (int t = 0; t < t_final + 1; t++) {
        ...
        length_x_growing_part = length_growing_initial * exp(t * strain(solution_grid -
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int xL = 0; xL < int(non_growing_final * double(solution_grid)); xL++) {
                cn(i, xL) = exp(-(D * i * i * M_PI * M_PI * t) / (length_constant_initial * length_constant_initial) +
                                t * (k_reac));
        ...
        for (int y = 0; y < solution_grid; y++) {
            ...
            sqrt_f2d2 = sqrt(f2 / d2);
            denom2 = sqrt_f2d2 * (-exp(-sqrt_f2d2 * length_constant_initial) +
                                   exp(sqrt_f2d2 * length_constant_initial -
                                       2 * sqrt_f2d2 * (length_x_growing_part)));


            if (t > 0) {

                cout << "chemo(gamma, t-1)" << chemo(gamma,t-1) << endl;
                cout << "chemo(gamma -1, t-1)" << chemo(gamma -1 ,t-1) << endl;

                A1 = (chemo(gamma , t - 1) - chemo(gamma -1, t - 1)) / denom1;
                A2 = (chemo(gamma -1, t - 1) - chemo(gamma, t - 1)) / denom2;
                cout << "denom 2 " << denom2 << endl;
                cout << "denom 1 " << denom1 << endl;
                ...
                if (y < gamma) {
                    ...
                    phi(y) = A1 * (exp(-sqrt_f1d1 * (double(y) / double(solution_grid))) +
                                   exp(sqrt_f1d1 * (double(y) / double(solution_grid))));
                } else {
                    phi(y) = A2 * (exp(-sqrt_f2d2 * (double(y) / double(solution_grid))) +
                                   exp(sqrt_f2d2 * (double(y) / double(solution_grid)) - 2 * sqrt_f2d2 * (length_x_growing_part)));
...
//                }
        ...
        for (int i = 0; i < solution_grid - int(non_growing_final * double(solution_grid)); i++) {
            grid_changes(i + int(non_growing_final * double(solution_grid)), t) =
                    i * length_x_growing_part / length_growing_initial + int(non_growing_final * double(solution_grid));
        }
