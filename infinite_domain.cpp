//
// Created by rasa on 13/12/18.
//
/*
 * Reaction-diffusion equation on a fixed domain, 1D, implicit method, different diffusion coefficients in different parts of the domain
 *
 * I should only look carefully at the domain theta_1 - 10 to theta_1_+10
 * */
...
//
        ...
        for (int i = 1; i < theta1 - 1; i++) {
            for (int j = 0; j < length_y; j++) {
                ai(i, j) = -D_1 * dt / (2.0 * Gamma_x(i) * dx * dx) * (1.0 / Gamma_x(i) + 1.0 / Gamma_x(i - 1));
        ...
        for (int i = 1; i < theta1 - 1; i++) {
            for (int j = 0; j < length_y; j++) {
                bi(i, j) =  (1 + dt *( strain(i) - k_reac) +
                 D_1 * dt / (2.0 * Gamma_x(i) * dx * dx) * (1.0 / Gamma_x(i) + 1.0 / Gamma_x(i + 1)) +
                 D_1 * dt / (2.0 * Gamma_x(i) * dx * dx) * (1.0 / Gamma_x(i) + 1.0 / Gamma_x(i - 1)));
                //cout << " bi " << bi << endl;
            }
        }
        ...
        // I will look at the dynamics somewhere in the middle
        for (int j = 0; j < length_y; j++) {
            ai(length_x - 1, j) = -2 * D_2 * dt / (2.0 * Gamma_x(length_x - 1) * dx * dx) *
                                  (1.0 / Gamma_x(length_x - 1) + 1.0 / Gamma_x(length_x - 2));
            bi(0, j) = (1 + dt *( strain(0) - k_reac) +
                        D_1 * dt / (2.0 * Gamma_x(0) * dx * dx) * (1.0 / Gamma_x(0) + 1.0 / Gamma_x(1)) +
                        D_1 * dt / (2.0 * Gamma_x(0) * dx * dx) * (1.0 / Gamma_x(0) + 1.0 / Gamma_x(1)));
            bi(length_x-1, j) = (1 + dt *( strain(length_x-1) - k_reac) +
                                   D_2 * dt / (2.0 * Gamma_x(length_x-1) * dx * dx) * (1.0 / Gamma_x(length_x-1) + 1.0 / Gamma_x(length_x-2)) +
                                   D_2 * dt / (2.0 * Gamma_x(length_x-1) * dx * dx) * (1.0 / Gamma_x(length_x-1) + 1.0 / Gamma_x(length_x-2)));
            gi(0, j) = -2 * D_1 * dt / (2.0 * Gamma_x(0) * dx * dx) * (1.0 / Gamma_x(0) + 1.0 / Gamma_x(1));
        }
