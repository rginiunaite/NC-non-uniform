...
for i = 1:3:N
    ...
    filename = sprintf('Domain growth points data/Domain075first05finalV2%i.000000.csv',i); % change after42
...
xlabel('Time, hrs')
 ...
 Gammatheta = values(round(342*0.5),:);% 
...
% 
  ...
  sigma1final = 0.0126182; % 50%slower
  sigma2final = 0.0254543; %50% grows faster
...
for  t = 0:1:N-1
    ...
    if t < 27
        ...
        a2(i) = secondpoint(1)*exp(sigma1*t);% + 342*theta1*(exp(sigma1*t) -exp(sigma2*t) ); % adapt as necessary  342 * 0.75 = 256. Leave when 0.5 and 0.25

        a3(i) = thirdpoint(1)*exp(sigma2*t)+342*theta1*(exp(sigma1*t) -exp(sigma2*t) );
...
%         if t == 27
%          total_length = (values(length(values(:,1)),1)*exp(sigma2final*t)+342*theta1*(exp(sigma1final*t) -exp(sigma2final*t) ));
%         total_length_before = (values(length(values(:,1)),1)*exp(sigma2*(t-1))+342*theta1*(exp(sigma1*(t-1)) -exp(sigma2*(t-1)) ));
%         total_increase = total_length - total_length_before;
%         scaling_factor = total_length/total_increase;
%         else
%         
%         total_length = (values(length(values(:,1)),1)*exp(sigma2final*t)+342*theta1*(exp(sigma1final*t) -exp(sigma2final*t) ));
%         total_length_before = (values(length(values(:,1)),1)*exp(sigma2final*(t-1))+342*theta1*(exp(sigma1final*(t-1)) -exp(sigma2final*(t-1)) ));
%         total_increase = total_length - total_length_before;
%         scaling_factor = total_length/total_increase;
%         end
        a1(i) = lastpoint1*exp(sigma1final*(t-lag));
