...
//
        ...
        for (int i = 0; i < theta1; i++) {
            Gamma(i) = Gamma_initial* (double(i) / double(space_grid_controller)) * (Gamma_x(i)); // linearly increasing
            ...
            for (int j=1; j < length_y -1; ++j){
                ...
                chemo_new(i,j) = dt * (D * 1.0 / (2.0 * dx * dx * Gamma_x(i)) *
                                       ((1.0 / Gamma_x(i) + 1.0 / Gamma_x(i + 1)) * (chemo(i + 1) - chemo(i)) -
                                        (chemo(i,j) - chemo(i - 1,j)) * (1.0 / Gamma_x(i) + 1.0 / Gamma_x(i - 1))) + D * (chemo(i,j+1) - 2 * chemo(i,j) + chemo(i,j-1))/(dy*dy) +
